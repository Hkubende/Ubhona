...
dishesRouter.post("/", async (req: AuthRequest, res) => {
  try {
    ...
    const billingRestaurant = restaurant as unknown as BillingRestaurantRecord;
    const dishLimit = await getRestaurantLimitStatus(billingRestaurant, "dishes");
    ...
    await incrementRestaurantUsage(billingRestaurant, "dishes", 1);
...
dishesRouter.delete("/:id", async (req: AuthRequest, res) => {
  try {
    ...
    await decrementRestaurantUsage(
      { id: tenantContext.restaurantId, ownerUserId: tenantContext.userId } as unknown as BillingRestaurantRecord,
      "dishes",
      1
    );
    await recordActivityEvent({
