...
export async function recordAnalyticsEvent(input: {
  ...
  return runWithPublicStorefrontDbContext(input.restaurantId, async () => {
    if (input.dishId) {
      const dish = await prisma.dish.findFirst({ where: { id: input.dishId }, select: { id: true } });
      if (!dish) throw new Error("Dish does not belong to this restaurant.");
    }
    if (input.orderId) {
      const order = await prisma.order.findFirst({ where: { id: input.orderId }, select: { id: true } });
