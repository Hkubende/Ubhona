...
export async function recordAnalyticsEvent(
  ...
  // Storefront analytics ingestion is anonymous by design, so we confirm the
  // restaurant exists first and then bind a synthetic public user inside the
  // tenant-scoped DB context before touching protected delegates.
  ...
  const createEvent = async () => {
    if (input.dishId) {
      const dish = await client.dish.findFirst({ where: { id: input.dishId }, select: { id: true } });
      if (!dish) throw new Error("Dish does not belong to this restaurant.");
    }
    if (input.orderId) {
      const order = await client.order.findFirst({ where: { id: input.orderId }, select: { id: true } });
  ...
  return runWithPublicStorefrontDbContext(input.restaurantId, createEvent);
}

export async function getAnalyticsSummary(input: TenantAnalyticsContext) {
  const { since, days: periodDays } = getSince(input.days);
  ...
  return runWithTenantContext({
    ...
    fn: async (tx) => {
      ...
      const dishes = dishIds.length
        ? await tx.dish.findMany({ where: { id: { in: dishIds } }, select: { id: true, name: true } })
        : [];
...
export async function getTopDishes(input: TenantAnalyticsContext) {
...
export async function getConversionMetrics(input: TenantAnalyticsContext) {
  const summary = await getAnalyticsSummary(input);
  return {
    periodDays: summary.periodDays,
