...
function resolvePath(target: Record<string, any>, path: string[]) {
  return path.reduce((current, key) => current[key], target);
}

function createDelegateProxy(basePrisma: PrismaLike, path: string[]): any {
  return new Proxy(
    {},
    {
        ...
        if (typeof value === "function") {
          return async (...args: any[]) => {
            ...
            if (currentContext?.tx) {
              return value.apply(resolvePath(currentContext.tx as Record<string, any>, path), args);
            }
            ...
            return basePrisma.$transaction(async (tx: PrismaLike) => {
              ...
              const delegate = resolvePath(tx as Record<string, any>, path);
              return runWithDbRlsContext({ ...currentContext, tx }, () => delegate[property](...args));
            });
          };
        }
