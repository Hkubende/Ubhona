...
export default function ARViewer() {
  ...
  const refreshMenu = React.useCallback(() => {
    fetchDishes()
      .then((data) => {
        if (!data.length) {
          setDishes([]);
          setToast("No dishes available. Add at least one available dish in Menu Manager.");
          setPanelNotice("No dishes are available right now. Please add available dishes in Menu Manager.");
      ...
      .catch(() => {
        setToast("Failed to load menu data. Please refresh or check dashboard setup.");
  ...
  React.useEffect(() => {
    ...
    const key = `mv_analytics_seen_${restaurantProfile.id}_page_view_ar`;
  ...
  React.useEffect(() => {
    ...
    if (restaurantProfile?.id) {
      const arSeenKey = `mv_analytics_seen_${restaurantProfile.id}_${selectedDish.id}_ar_open`;
  ...
  }, [selectedDish, setSearchParams, searchParams, initializedFromUrl, restaurantProfile?.id]);
    ...
    if (restaurantProfile?.id) {
      void trackAnalyticsEvent({
        ...
        metadata: {
          ...
          total: getCartTotal(cart, dishes, getDishPrice),
        },
      });
    }
    setPanelNotice("");
        ...
        <div className="fixed left-1/2 top-1/2 z-[90] w-[min(760px,94vw)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-3xl border border-white/10 bg-[#101017]/95 shadow-2xl shadow-black/50 backdrop-blur-xl">
          <div className="flex items-center justify-between border-b border-white/10 bg-black/30 px-5 py-4">
            <div className="text-lg font-black tracking-tight">
              Checkout <span className="text-orange-400">Ubhona</span>
            </div>
            <button
