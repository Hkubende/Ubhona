...
export default function Checkout() {
  ...
  const completeOrder = async (paymentMethod: "stk_push" | "manual_mpesa", paymentReference: string) => {
    try {
      const order = await createAndStoreOrderFromCart(
        cart,
        dishes,
        getDishPrice,
        paymentMethod,
      ...
      setNotice(error instanceof Error ? error.message : "Failed to place order.");
      return false;
    }
  };
    ...
    try {
      ...
      if (res.ok && body?.ok) {
        ...
        const ref = typeof body?.checkoutRequestId === "string" && body.checkoutRequestId
          ...
          : pendingOrderId;
        void completeOrder("stk_push", ref);
      } else {
  ...
  const confirmManualPayment = () => {
    ...
    void completeOrder("manual_mpesa", pendingOrderId);
  };
